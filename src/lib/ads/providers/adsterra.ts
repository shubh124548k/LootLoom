...
export class AdsterraProvider extends BaseAdProvider {
  ...
  get scriptUrl(): string | null {
    return "https://pl30349373.effectivecpmnetwork.com/2e/88/bc/2e88bc28cdbfdafc4b85833ebade6a5c.js";
  }
  ...
  protected async showAd(_userId: string, _sessionId: string): Promise<{ success: boolean; error?: string }> {
    ...
    const adsterraSdk = (window as any).Adsterra || (window as any).adsterra;

    if (typeof adsterraSdk?.showRewarded === "function") {
      try {
        await Promise.resolve(adsterraSdk.showRewarded());
