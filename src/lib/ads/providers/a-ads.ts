...
export class AAdsProvider extends BaseAdProvider {
  constructor() {
    super("a-ads", "A-Ads");
  }
  ...
  get scriptUrl(): string | null {
    ...
    return `https://a-ads.com/api/script/${this.config.publisherId}`;
  }

  protected async showAd(_userId: string, _sessionId: string): Promise<{ success: boolean; error?: string }> {
    return new Promise((resolve) => {
      const sdk = (window as any).AAds || (window as any).aads;
