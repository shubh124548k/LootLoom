...
export class AdRevenueProvider extends BaseAdProvider {
  constructor() { super("adrevenue", "AdRevenue"); }

  get scriptUrl(): string | null {
    if (!this.config?.publisherId) return null;
    return `https://adrevenue.net/${this.config.publisherId}`;
  }

  protected async showAd(_userId: string, _sessionId: string): Promise<{ success: boolean; error?: string }> {
    const sdk = (window as any).AdRevenue;
