...
export class EvadavProvider extends BaseAdProvider {
  constructor() { super("evadav", "Evadav"); }

  get scriptUrl(): string | null {
    if (!this.config?.publisherId) return null;
    return `https://evadav.com/${this.config.publisherId}`;
  }

  protected async showAd(_userId: string, _sessionId: string): Promise<{ success: boolean; error?: string }> {
    const sdk = (window as any).Evadav;
