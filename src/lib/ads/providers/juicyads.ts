...
export class JuicyAdsProvider extends BaseAdProvider {
  constructor() { super("juicyads", "JuicyAds"); }

  get scriptUrl(): string | null {
    if (!this.config?.publisherId) return null;
    return `https://juicyads.com/script/${this.config.publisherId}`;
  }

  protected async showAd(_userId: string, _sessionId: string): Promise<{ success: boolean; error?: string }> {
    const sdk = (window as any).JuicyAds;
