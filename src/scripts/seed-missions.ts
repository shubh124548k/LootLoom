...
async function main() {
  ...
  for (const mission of missions) {
    const existing = await db.mission.findUnique({ where: { key: mission.key } });
    if (!existing) {
      await db.mission.create({ data: mission });
      console.log(`  + Created: ${mission.name}`);
    } else {
      console.log(`  = Already exists: ${mission.name}`);
    }
  }

  console.log("Seeding earning config defaults...");
  const configs = [
    { key: "AD_REWARD_AMOUNT", value: "25", label: "Coins per rewarded ad", type: "NUMBER" },
    { key: "DAILY_AD_LIMIT", value: "100", label: "Max ads per day", type: "NUMBER" },
  ...
  for (const cfg of configs) {
    ...
    if (!existing) {
      ...
      console.log(`  + Created config: ${cfg.key} = ${cfg.value}`);
    } else {
      console.log(`  = Already exists config: ${cfg.key} = ${existing.value}`);
    }
  }
